...
export default function MatcherView() {
  ...
  const weightTotal = preferences.skillsWeight + preferences.experienceWeight + preferences.educationWeight + preferences.overallWeight;
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      ...
      {activeStep === 2 && (
        <div className={`${bg} p-6 space-y-5`}>
          ...
          <FileDropZone label="Drop resumes here" accept=".pdf,.docx,.txt" multiple={true} files={resumeFiles} onFilesChange={setResumeFiles} hint="Upload multiple resumes — PDF, DOCX, or TXT" icon={Users} />
          {resumeFiles.length > 0 && (
            <div className={`flex items-center gap-2 rounded-xl p-3 ${darkMode ? 'bg-teal-900/30 border border-teal-800' : 'bg-teal-50 border border-teal-200'}`}>
              <Users className="w-4 h-4 text-teal-600 shrink-0" />
              <p className={`text-sm ${darkMode ? 'text-teal-300' : 'text-teal-700'}`}>
                <span className="font-semibold">{resumeFiles.length}</span> resume{resumeFiles.length !== 1 ? 's' : ''} ready
              </p>
            </div>
          )}
