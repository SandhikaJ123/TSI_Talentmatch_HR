          ...
          {aiInsights && showFullInsights && (
            <div className={`rounded-xl border-2 border-violet-200 ${darkMode ? 'bg-violet-950/20' : 'bg-violet-50'} p-5 space-y-4`}>
              ...
              {aiInsights.overallAssessment && (
                <div>
                  <h4 className={`text-sm font-semibold uppercase tracking-wider mb-2 ${muted}`}>Overall Assessment</h4>
              ...
              {aiInsights.recommendation && (
                <div className={`rounded-lg p-4 border-2 ${
                  aiInsights.recommendation.startsWith('HIRE')    ? 'bg-emerald-50 border-emerald-400' :
                  aiInsights.recommendation.startsWith('PASS')    ? 'bg-red-50 border-red-400' :
                                                                    'bg-amber-50 border-amber-400'
                }`}>
                  <div className="flex items-center gap-2 mb-1">
                    <Target className={`w-4 h-4 ${
                      aiInsights.recommendation.startsWith('HIRE')    ? 'text-emerald-600' :
                      aiInsights.recommendation.startsWith('PASS')    ? 'text-red-600' :
                                                                        'text-amber-600'
                    }`} />
                    <h4 className={`text-sm font-bold uppercase tracking-wider ${
                      aiInsights.recommendation.startsWith('HIRE')    ? 'text-emerald-700' :
                      aiInsights.recommendation.startsWith('PASS')    ? 'text-red-700' :
                                                                        'text-amber-700'
                    }`}>Recommendation</h4>
                  </div>
                  <p className="text-sm leading-relaxed font-medium">{aiInsights.recommendation}</p>
                </div>
              )}
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-sm font-semibold uppercase ${
                      ...
                      aiInsights.hiringRisk === 'medium' ? 'text-yellow-700' :
                                                           'text-red-700'
                    }`}>{aiInsights.hiringRisk} Risk</span>
                  </div>
                  {aiInsights.riskFactors?.length > 0 && (
                    <ul className="space-y-1 ml-6">
                      {aiInsights.riskFactors.map((risk, i) => (
                        <li key={i} className={`text-xs ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>• {risk}</li>
                      ))}
                    </ul>
                  )}
                </div>
              ...
              {/* Development Plan */}
              {aiInsights.developmentPlan?.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-1.5">
                    {aiInsights.developmentPlan.map((step, i) => (
              ...
              {/* ── Stored data from extraction (no AI re-call) ── */}
              {storedData && (
                <div className={`border-t pt-4 space-y-4 ${border}`}>

                  {/* Summary */}
                  {storedData.summary && (
                    <div>
                      <h4 className={`text-sm font-semibold uppercase tracking-wider mb-1 ${muted}`}>AI Summary</h4>
                      <p className="text-sm leading-relaxed">{storedData.summary}</p>
                    </div>
                  )}

                  {/* Strengths */}
                  {storedData.strengths?.length > 0 && (
                    <div>
                      <h4 className={`text-sm font-semibold uppercase tracking-wider mb-2 flex items-center gap-1 ${muted}`}>
                        <CheckCircle className="w-4 h-4 text-emerald-600" /> Strengths
                      </h4>
                      <ul className="space-y-1">
                        {storedData.strengths.map((s, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm">
                  ...
                  {/* Weaknesses */}
                  {storedData.weaknesses?.length > 0 && (
                    <div>
                      <h4 className={`text-sm font-semibold uppercase tracking-wider mb-2 flex items-center gap-1 ${muted}`}>
                        <AlertTriangle className="w-4 h-4 text-orange-500" /> Gaps / Weaknesses
                      </h4>
                      <ul className="space-y-1">
                        {storedData.weaknesses.map((w, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm">
                  ...
                  {storedData.interviewFocusAreas?.length > 0 && (
                    <div>
                      <h4 className={`text-sm font-semibold uppercase tracking-wider mb-2 flex items-center gap-1 ${muted}`}>
                        <Target className="w-4 h-4 text-rose-500" /> Interview Focus Areas
                      </h4>
                      <ul className="space-y-1.5">
                        {storedData.interviewFocusAreas.map((area, i) => {
                          const [label, ...rest] = area.split(':');
                          const detail = rest.join(':').trim();
                          return (
                            <li key={i} className="flex items-start gap-2 text-sm">
                              <span className="text-rose-500 font-bold shrink-0">{i + 1}.</span>
                              <span>{detail ? <><strong>{label.trim()}:</strong> {detail}</> : area}</span>
                            </li>
                          );
                        })}
                      </ul>
                  ...
                  {/* Interview Questions */}
                  {storedData.interviewQuestions?.length > 0 && (
                    <div>
                      <h4 className={`text-sm font-semibold uppercase tracking-wider mb-2 flex items-center gap-1 ${muted}`}>
                        <MessageSquare className="w-4 h-4 text-violet-500" /> Gap-Focused Interview Questions
                      </h4>
                      <ul className="space-y-2">
                        {storedData.interviewQuestions.map((q, i) => (
                          <li key={i} className="flex items-start gap-2 text-sm">
            ...
            {[
              ...
              return (
                <div key={key} className={`rounded-xl p-4 border ${border} ${cardBg}`}>
                  ...
                  {key === 'experience' && <p className={`text-xs mt-1 ${muted}`}>{(() => { const y = data.detectedYears ?? data.years; return y < 1 && y > 0 ? `${Math.round(y * 12)}m` : y ? `${y}y` : '—'; })()} detected</p>}
