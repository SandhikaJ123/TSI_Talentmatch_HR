...
import { ChevronDown, ChevronUp, Award, Briefcase, GraduationCap, Zap, Tag, Sparkles, Lightbulb, Target, MessageSquare, Download } from 'lucide-react';const GRADE_STYLES = {
...
export default function ResultCard({ result, rank, darkMode }) {
  ...
  const handleDownloadQuestions = async (e) => {
    e.stopPropagation();
    if (!result.interviewQuestions?.length) return;
    ...
    try {
      ...
      a.href = url;
      a.download = `${result.name.replace(/\s+/g, '_')}_Interview_Guide.pdf`;
    ...
    <div className={`rounded-xl border border-l-4 ${style.border} ${cardBg} shadow-sm hover:shadow-md transition-shadow overflow-hidden`}>
      ...
      {expanded && (
        <div className={`border-t p-4 space-y-4 ${expandBg}`}>
              ...
              return (
                <div key={key} className={`rounded-lg p-3 border ${innerCard}`}>
                  ...
                  <div className="flex items-end justify-between mb-1.5">
                    ...
                    {key === 'experience' && <span className={`text-xs ${textMuted}`}>{fmtExp(data.years ?? data.detectedYears)}</span>}
          ...
          {result.breakdown?.custom?.length > 0 && (
            <div className="space-y-2">
              <p className={`text-xs font-semibold uppercase tracking-wider ${textMuted}`}>Custom Criteria</p>
              {result.breakdown.custom.map((c) => (
                <div key={c.id} className={`rounded-lg p-3 border ${innerCard}`}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-1.5">
                      <Lightbulb className="w-3.5 h-3.5 text-purple-500" />
                      <span className={`text-sm font-medium ${textMain}`}>{c.term}</span>
                    </div>
                    <span className={`text-sm font-bold ${c.matched ? 'text-emerald-600' : 'text-slate-400'}`}>
                      {c.score}%
                    </span>
                  </div>
                  <div className={`h-1 rounded-full overflow-hidden mb-1.5 ${darkMode ? 'bg-slate-700' : 'bg-slate-100'}`}>
                    <div className="h-full rounded-full bg-purple-500" style={{ width: `${c.score}%` }} />
                  </div>
                  {c.evidence && <p className={`text-xs ${textMuted}`}>{c.evidence}</p>}
                </div>
              ))}
            </div>
          )}
          ...
          {result.breakdown?.experience && (
            <div className={`text-xs ${textMuted} pt-2 border-t ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
              <strong>Experience Details:</strong> {fmtExp(result.breakdown.experience.detectedYears)} detected
              {result.breakdown.experience.requiredYears && ` (${result.breakdown.experience.requiredYears}y required)`}
            </div>
          )}
          {/* Interview Focus Areas — always visible */}
          {result.interviewFocusAreas?.length > 0 && (
            <div className={`rounded-lg p-3 border ${darkMode ? 'bg-amber-900/20 border-amber-800' : 'bg-amber-50 border-amber-200'}`}>
              <div className="flex items-center gap-1.5 mb-2">
              ...
              <ul className="space-y-1.5">
                {result.interviewFocusAreas.map((area, i) => {
                  const [label, ...rest] = area.split(':');
                  const detail = rest.join(':').trim();
                  return (
                    <li key={i} className="flex items-start gap-2">
                      <span className={`text-xs font-bold mt-0.5 shrink-0 ${darkMode ? 'text-amber-400' : 'text-amber-500'}`}>{i + 1}.</span>
                      <span className={`text-xs leading-relaxed ${darkMode ? 'text-amber-200' : 'text-amber-700'}`}>
                        {detail ? <><strong>{label.trim()}:</strong> {detail}</> : area}
                      </span>
                    </li>
                  );
                })}
          ...
          {/* Gap-focused Interview Questions — collapsible */}
          {result.interviewQuestions?.length > 0 && (
            <div className={`rounded-lg p-3 border ${darkMode ? 'bg-violet-900/20 border-violet-800' : 'bg-violet-50 border-violet-200'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <MessageSquare className="w-4 h-4 text-violet-600" />
                  <p className={`text-sm font-semibold ${darkMode ? 'text-violet-300' : 'text-violet-800'}`}>
                    Gap-Focused Interview Questions
                  </p>
                </div>
                <button
                  onClick={() => setShowQuestions(!showQuestions)}
                  className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors border shadow-sm ${
                    showQuestions
                      ? (darkMode ? 'bg-violet-800/80 text-violet-100 border-violet-700' : 'bg-violet-200 text-violet-900 border-violet-300')
                      : (darkMode ? 'bg-violet-900/50 text-violet-300 border-violet-700 hover:bg-violet-800/70' : 'bg-white text-violet-700 border-violet-200 hover:bg-violet-50')
                  }`}
                >
                  {showQuestions ? 'Hide' : `Show (${result.interviewQuestions.length})`}
                </button>
              </div>

              {showQuestions && (
                <div className={`mt-3 pt-3 border-t ${darkMode ? 'border-violet-800/50' : 'border-violet-200/60'}`}>
                  <div className="flex items-center justify-end mb-2">
                    <button
                      onClick={handleDownloadQuestions}
                      className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md transition-colors border ${
                        darkMode
                          ? 'text-violet-300 border-violet-700 hover:text-violet-100 hover:bg-violet-800/50'
                          : 'text-violet-700 border-violet-300 hover:text-violet-900 hover:bg-violet-200/60'
                      }`}
                    >
                  ...
                  <ul className="space-y-2.5">
                    {result.interviewQuestions.map((q, idx) => (
                      <li key={idx} className={`text-sm flex items-start gap-2.5 ${darkMode ? 'text-violet-200' : 'text-violet-700'}`}>
                        <span className={`font-bold mt-[1px] shrink-0 ${darkMode ? 'text-violet-500' : 'text-violet-500'}`}>{idx + 1}.</span>
