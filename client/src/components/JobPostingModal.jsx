...
export default function JobPostingModal({ onClose, onJobCreated, darkMode }) {
  ...
  const isValid = mode === 'paste' ? description.trim().length >= 30 : uploadFile;
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Create job with all parsed fields
      const result = await createJob({
        title:            parseResult.parsed.title || 'Untitled Position',
        department:       parseResult.parsed.department || 'Engineering',
        location:         parseResult.parsed.location || '',
        type:             parseResult.parsed.type || 'Full-time',
        description:      jobDescription,
        summary:          parseResult.parsed.summary || '',
        minExperience:    parseResult.parsed.minExperience || 0,
        educationLevel:   parseResult.parsed.educationLevel || '',
        requiredSkills:   parseResult.parsed.requiredSkills || [],
        niceToHaveSkills: parseResult.parsed.niceToHaveSkills || [],
        responsibilities: parseResult.parsed.responsibilities || [],
        salary:           parseResult.parsed.salary || '',
        parsedBy:         parseResult.parsed.parsedBy || 'nlp',
      });
