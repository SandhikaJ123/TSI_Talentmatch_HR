...
async def parse_job_endpoint(
    ...
    use_ai = useAI != "false" and bool(os.getenv("OPENAI_API_KEY") or os.getenv("ANTHROPIC_API_KEY"))
...
async def create_job(body: dict, background_tasks: BackgroundTasks):
    ...
    async def _vectorize():
        try:
            ...
            if vector is None:
                print(f"⚠ Skipping embedding for \"{title}\" — no OpenAI key configured")
                return
...
async def vectorize_job(job_id: str):
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise HTTPException(400, "Vectorization requires OPENAI_API_KEY to be configured.")
    job = dict(row)
